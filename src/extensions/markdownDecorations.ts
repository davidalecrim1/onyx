...
const tagMark = Decoration.mark({ class: "onyx-tag" });
    ...
    enter(node) {
      ...
      switch (node.type.name) {
        ...
        case "FencedCode": {
          ...
          for (const child of node.node.cursor().node.getChildren("CodeMark")) {
        ...
        case "Link": {
          ...
          if (!cursorIsHere) {
            ...
            let closeBracketAndUrl: { from: number; to: number } | null = null;
...
function safeBuildDecorations(state: EditorState): DecorationSet {
  try {
    return buildDecorations(state);
  } catch (error) {
    console.error("markdownDecorations: buildDecorations failed", error);
...
export const markdownDecorations = StateField.define<DecorationSet>({
  ...
  update(decorations, transaction) {
    if (transaction.docChanged || transaction.selection) {
      return safeBuildDecorations(transaction.state);
    }
    return decorations;
  },
  provide(field) {
