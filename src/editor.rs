...
impl Tab {
    ...
    /// Returns the current Vim mode, forwarded from the underlying VimEngine.
    pub fn mode(&self) -> crate::vim::Mode {
        self.editor.mode()
    }
...
impl Editor {
    ...
    pub fn handle_key(&mut self, key: Key, viewport_lines: usize) {
        if let Some(cmd) = self.vim.handle_key(key) {
            self.apply(cmd, viewport_lines);
        }
    }

    fn apply(&mut self, cmd: BufferCommand, viewport_lines: usize) {
        match cmd {
        ...
        self.buffer.clamp_scroll(viewport_lines);
    }
...
mod tests {
    ...
    fn typing_in_insert_mode_modifies_buffer() {
        ...
        ed.handle_key(Key::Char('i'), 24);
        ed.handle_key(Key::Char('!'), 24);
